...
Serial::SerialImpl::~SerialImpl() { this->close(); }
...
void Serial::SerialImpl::reconfigurePort()
{
  ...
  // setup char len
  if( bytesize_ == eightbits ) dcbSerialParams.ByteSize = 8;
  ...
  else if( parity_ == parity_even ) { dcbSerialParams.Parity = EVENPARITY; }
  else if( parity_ == parity_odd ) { dcbSerialParams.Parity = ODDPARITY; }
  else if( parity_ == parity_mark ) { dcbSerialParams.Parity = MARKPARITY; }
  ...
  if( !SetCommTimeouts( fd_, &timeouts ) ) { THROW( IOException, "Error setting timeouts." ); }
}
...
void Serial::SerialImpl::waitByteTimes( size_t /*count*/ ) { THROW( IOException, "waitByteTimes is not implemented on Windows." ); }
...
serial_cpp::Timeout Serial::SerialImpl::getTimeout() const { return timeout_; }
...
void Serial::SerialImpl::setBytesize( serial_cpp::bytesize_t bytesize )
{
  bytesize_ = bytesize;
  if( is_open_ ) { reconfigurePort(); }
}

serial_cpp::bytesize_t Serial::SerialImpl::getBytesize() const { return bytesize_; }

void Serial::SerialImpl::setParity( serial_cpp::parity_t parity )
{
  parity_ = parity;
  if( is_open_ ) { reconfigurePort(); }
}

serial_cpp::parity_t Serial::SerialImpl::getParity() const { return parity_; }

void Serial::SerialImpl::setStopbits( serial_cpp::stopbits_t stopbits )
{
  stopbits_ = stopbits;
  if( is_open_ ) { reconfigurePort(); }
}

serial_cpp::stopbits_t Serial::SerialImpl::getStopbits() const { return stopbits_; }

void Serial::SerialImpl::setFlowcontrol( serial_cpp::flowcontrol_t flowcontrol )
{
  flowcontrol_ = flowcontrol;
  if( is_open_ ) { reconfigurePort(); }
}

serial_cpp::flowcontrol_t Serial::SerialImpl::getFlowcontrol() const { return flowcontrol_; }
...
void Serial::SerialImpl::sendBreak( int /*duration*/ ) { THROW( IOException, "sendBreak is not supported on Windows." ); }

void Serial::SerialImpl::setBreak( bool level )
{
  if( is_open_ == false ) { throw PortNotOpenedException( "Serial::setBreak" ); }
  if( level ) { EscapeCommFunction( fd_, SETBREAK ); }
  else
  {
...
void Serial::SerialImpl::setRTS( bool level )
{
  ...
  if( level ) { EscapeCommFunction( fd_, SETRTS ); }
  else
  {
...
void Serial::SerialImpl::setDTR( bool level )
{
  ...
  if( level ) { EscapeCommFunction( fd_, SETDTR ); }
  else
  {
...
{
  if( is_open_ == false ) { throw PortNotOpenedException( "Serial::waitForChange" ); }
  DWORD dwCommEvent;
...
bool Serial::SerialImpl::getCTS()
{
  ...
  DWORD dwModemStatus;
  if( !GetCommModemStatus( fd_, &dwModemStatus ) ) { THROW( IOException, "Error getting the status of the CTS line." ); }
...
bool Serial::SerialImpl::getDSR()
{
  ...
  DWORD dwModemStatus;
  if( !GetCommModemStatus( fd_, &dwModemStatus ) ) { THROW( IOException, "Error getting the status of the DSR line." ); }
...
bool Serial::SerialImpl::getRI()
{
  ...
  DWORD dwModemStatus;
  if( !GetCommModemStatus( fd_, &dwModemStatus ) ) { THROW( IOException, "Error getting the status of the RI line." ); }
