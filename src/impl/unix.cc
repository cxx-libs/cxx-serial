...
Serial::SerialImpl::~SerialImpl() { close(); }
...
void Serial::SerialImpl::reconfigurePort()
{
  ...
  if( parity_ == parity_none ) { options.c_cflag &= ( tcflag_t ) ~( PARENB | PARODD ); }
  ...
  #ifdef CMSPAR
  else if( parity_ == parity_mark ) { options.c_cflag |= ( PARENB | CMSPAR | PARODD ); }
  ...
  else if( parity_ == parity_mark || parity_ == parity_space ) { throw invalid_argument( "OS does not support mark or space parity" ); }
  #endif  // ifdef CMSPAR
  else
  {
  ...
  // xonxoff
  #ifdef IXANY
  if( xonxoff_ ) options.c_iflag |= ( IXON | IXOFF );  //|IXANY)
  else
  ...
  if( custom_baud == true )
  {
    ...
    if( -1 == ioctl( fd_, IOSSIOSPEED, &new_baud, 1 ) ) { THROW( IOException, errno ); }
      // Linux Support
  #elif defined( __linux__ ) && defined( TIOCSSERIAL )
    struct serial_struct se;

    if( -1 == ioctl( fd_, TIOCGSERIAL, &se ) ) { THROW( IOException, errno ); }

    // set custom divisor
    se.custom_divisor = se.baud_base / static_cast<int>( baudrate_ );
    // update flags
    se.flags &= ~ASYNC_SPD_MASK;
    se.flags |= ASYNC_SPD_CUST;

    if( -1 == ioctl( fd_, TIOCSSERIAL, &se ) ) { THROW( IOException, errno ); }
  #else
  ...
  // and not 1.5.
  if( stopbits_ == stopbits_one_point_five ) { byte_time_ns_ += ( 1.5 - static_cast<double>( stopbits_one_point_five ) ) * bit_time_ns; }
}
  ...
  while( bytes_read < size )
  {
    ...
    if( waitReadable( timeout ) )
    {
      ...
      if( size > 1 && timeout_.inter_byte_timeout == Timeout::max() )
      {
        ...
        if( bytes_available + bytes_read < size ) { waitByteTimes( size - ( bytes_available + bytes_read ) ); }
      }
  ...
  while( bytes_written < length )
  {
    ...
    if( r > 0 )
    {
      ...
      if( FD_ISSET( fd_, &writefds ) )
      {
        ...
        if( bytes_written_now == -1 && errno == EINTR ) { continue; }
