...
bool read_response( serial_cpp::Serial& serial, std::vector<uint8_t>& buf )
{
  ...
  while( true )
  {
    ...
    if( std::chrono::duration_cast<std::chrono::milliseconds>( now - start ).count() > 2000 ) return false;  // timeout
    ...
    if( n == 1 )
    {
      ...
      if( expected_size > 0 && buf.size() >= expected_size ) return true;  // full response received
    }
  }
}
...
float parse_float32( const std::vector<uint8_t>& resp )
{
  ...
  uint32_t raw = ( uint32_t( resp[3] ) << 24 ) | ( uint32_t( resp[4] ) << 16 ) | ( uint32_t( resp[5] ) << 8 ) | ( uint32_t( resp[6] ) );
  float    value;
...
std::string parse_string( const std::vector<uint8_t>& resp )
{
  ...
  while( !out.empty() && ( out.back() < 32 || out.back() > 126 ) ) out.pop_back();
...
int main()
{
  serial_cpp::Serial serial( "COM5", 9600, serial_cpp::Timeout::simpleTimeout( 1000 ), serial_cpp::eightbits, serial_cpp::parity_none, serial_cpp::stopbits_two, serial_cpp::flowcontrol_none );
  ...
  while( true )
  {
    ...
    if( read_registers( serial, slave, 0x0000, 2, resp ) ) gas_flow = parse_float32( resp );

    if( read_registers( serial, slave, 0x0002, 2, resp ) ) temperature = parse_float32( resp );

    if( read_registers( serial, slave, 0x0004, 2, resp ) ) totaliser = parse_float32( resp );

    if( read_registers( serial, slave, 0x6046, 4, resp ) ) measuring_unit = parse_string( resp );
